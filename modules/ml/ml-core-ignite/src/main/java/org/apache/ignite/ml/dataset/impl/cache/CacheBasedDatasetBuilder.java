...
public class CacheBasedDatasetBuilder<K, V> implements DatasetBuilder<K, V> {
     ...
     */
    public CacheBasedDatasetBuilder(Ignite ignite, IgniteCache<K, V> upstreamCache, SerializableBiPredicate<K, V> filter) {
    ...
    @Override public DatasetBuilder<K, V> withFilter(SerializableBiPredicate<K, V> filterToAdd) {
        return new CacheBasedDatasetBuilder<>(ignite, upstreamCache,
            (e1, e2) -> filter.test(e1, e2) && filterToAdd.test(e1, e2));
    }
}
