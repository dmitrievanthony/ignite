...
public class CacheBasedLabelPairCursor<L, K, V> implements LabelPairCursor<L> {
     ...
     */
    public CacheBasedLabelPairCursor(IgniteCache<K, V> upstreamCache, SerializableBiPredicate<K, V> filter,
     ...
     * @return Query cursor.
     */
    private QueryCursor<Cache.Entry<K, V>> query(IgniteCache<K, V> upstreamCache, SerializableBiPredicate<K, V> filter) {
        ScanQuery<K, V> qry = new ScanQuery<>();

        if (filter != null) // This section was added to keep code correct of qry.setFilter(null) behaviour will changed.
            qry.setFilter(filter::test);
