...
public class CrossValidation<M extends IgniteModel<Vector, L>, L, K, V> {
     ...
     */
    public CrossValidationResult score(DatasetTrainer<M, L> trainer,
        Metric<L> scoreCalculator,
        Function<SerializableBiPredicate<K, V>, DatasetBuilder<K, V>> datasetBuilderSupplier,
        BiFunction<SerializableBiPredicate<K, V>, M, LabelPairCursor<L>> testDataIterSupplier,
        ...
        paramSets.forEach(paramSet -> {
            ...
            double[] locScores = score(trainer, scoreCalculator, datasetBuilderSupplier, testDataIterSupplier, filter,
                featureExtractor, lbExtractor, cv);
     ...
     * @return Array of scores of the estimator for each run of the cross validation.
     */
    public double[] score(DatasetTrainer<M, L> trainer,
        Metric<L> scoreCalculator,
        Function<SerializableBiPredicate<K, V>, DatasetBuilder<K, V>> datasetBuilderSupplier,
        BiFunction<SerializableBiPredicate<K, V>, M, LabelPairCursor<L>> testDataIterSupplier,
        IgniteBiFunction<K, V, Vector> featureExtractor,
        IgniteBiFunction<K, V, L> lbExtractor,
        int cv) {
        return score(trainer, scoreCalculator, datasetBuilderSupplier, testDataIterSupplier, (k, v) -> true, featureExtractor,
            lbExtractor, cv);
    }
    ...
    public double[] score(DatasetTrainer<M, L> trainer,
        ...
        IgniteBiFunction<K, V, L> lbExtractor,
        int cv) {
        return score(trainer, scoreCalculator, datasetBuilderSupplier, testDataIterSupplier, filter, featureExtractor,
            lbExtractor, new SHA256UniformMapper<>(), cv);
    }
    ...
    public double[] score(DatasetTrainer<M, L> trainer,
        ...
        return score(
            trainer,
            predicate -> datasetBuilderSupplier.apply((k, v) -> filter.test(k, v) && predicate.test(k, v)),
            (predicate, mdl) -> testDataIterSupplier.apply((k, v) -> filter.test(k, v) && !predicate.test(k, v), mdl),
            featureExtractor,
            lbExtractor,
            scoreCalculator,
            mapper,
    ...
    private double[] score(DatasetTrainer<M, L> trainer,
        ...
        for (int i = 0; i < cv; i++) {
            ...
            SerializableBiPredicate<K, V> trainSetFilter = (k, v) -> {
